...
```{r}
ggplot(surveys, aes(x = weight, y = hindfoot_length)) + geom_point(alpha = 0.2)
```
...
```{r}
ggplot(surveys, aes(x = weight, y = hindfoot_length)) + geom_point(alpha = 0.2) + facet_wrap(~ species)
```

**7. No plot is complete without axis labels and a title. Include reader friendly labels and a title to your plot.**

```{r}
ggplot(surveys, aes(x = weight, y = hindfoot_length)) + geom_point(alpha = 0.2) + facet_wrap(~ species) + labs(x = "Weight (grams)", y = "Hindfoot Length (mm)", title = "Scatterplot of Weight vs Hindfoot Length Faceted by Species")
```
...
```{r}
ggplot(surveys, aes(x = weight, y = hindfoot_length)) + geom_point(alpha = 0.2) + facet_wrap(~ species) + labs(x = "Weight (grams)", y = "", title = "Scatterplot of Weight vs Hindfoot Length Faceted by Species", subtitle = "Hindfoot Length (mm)")
```
...
```{r}
#| label: boxplot
ggplot(surveys, aes(x = species, y = weight)) + geom_boxplot()
```
...
```{r}
ggplot(surveys, aes(x = species, y = weight)) + geom_boxplot() + geom_point()
```
...
```{r}
ggplot(surveys, aes(x = species, y = weight)) + geom_boxplot() + geom_jitter()
```
...
```{r}
ggplot(surveys, aes(x = species, y = weight)) + geom_boxplot() + geom_jitter(color = "orange3", alpha = 0.2)
```
...
```{r}
ggplot(surveys, aes(x = species, y = weight)) + geom_boxplot(outliers = FALSE) + geom_jitter(color = "orange3", alpha = 0.2)
```
...
ggplot(surveys, aes(x = weight, y = species)) + geom_boxplot(outliers = FALSE) + geom_jitter(color = "orange3", alpha = 0.2) + labs(x = "Weight (grams)", y = "Species") + theme(axis.text.x = element_text(angle = 45))
```
...
Since the P-value from the ANOVA F-test was <2e-16, we can reject the null hypothesis and can say that there is a statistically significant difference of weight for at least one rodent species.
